...
export async function POST(req: NextRequest) {
  try {
    ...
    const sql = getDb();
    const c = (await sql`SELECT id, tipo, numero, total, moneda, cliente_id, cliente_nombre, leyendas, afip_cae, datos_recibo FROM fg_comprobantes WHERE token=${token} LIMIT 1` as any[])[0];
    ...
    if (c.cliente_id) {
      const cl = await sql`SELECT email, nombre, razon_social, cuit FROM clientes WHERE id=${c.cliente_id} LIMIT 1` as any[];
      email = cl[0]?.email || "";
      nombre = cl[0]?.nombre || cl[0]?.razon_social || nombre;          // nombre de contacto (para el saludo)
      razonSocial = cl[0]?.razon_social || cl[0]?.nombre || razonSocial; // razón social (para el cuerpo)
      cuit = cl[0]?.cuit || "";
    }
    ...
    const r = await fetch("https://febecos.com/api/admin?action=enviar-comprobante", {
      method: "POST", headers,
      body: JSON.stringify({
        email, nombre, razon_social: razonSocial, cuit,
        numero: c.numero, tipo: c.tipo, proforma: !c.afip_cae,
        total: c.total, moneda: c.moneda, link, leyendas: c.leyendas || [],
        ...(c.tipo === 'recibo' && c.datos_recibo ? (() => {
          const dr = typeof c.datos_recibo === 'string' ? JSON.parse(c.datos_recibo) : c.datos_recibo;
          return { saldo: dr.saldo, total_cobrar: dr.total_cobrar, total_pagado: dr.total_pagado, total_label: dr.total_label };
        })() : {}),
      }),
    });
