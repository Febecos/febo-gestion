            ...
            return (
              <div className="border border-gray-200 rounded-lg p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <a href={`/etiquetas/${encodeURIComponent(ped.numero)}`} target="_blank" rel="noreferrer" title="Genera etiquetas de envío (4 por hoja A4) con remitente FEBECOS y los datos del destinatario — editables antes de imprimir" className="px-3 py-1.5 rounded-lg border border-violet-300 text-violet-700 text-sm font-semibold hover:bg-violet-50">🏷️ Etiquetas de envío</a>
                </div>
...
function RemitoPanel({ items, remitos, despachoCompleto, despachoConfirmado, legacyNumero, legacyToken, envioCompleto, transporteOk, transporteNombre, facturado, pagadoOk, busy, valorBase, onGenerar, onRegenerar, onEliminar, onConfirmar, onEnviarConfirmacion, onEliminarConfirmacion, remitosExternos, onRemitoExterno, onEliminarRemitoExterno, onEnviarRemitoExterno }: any) {
  ...
  const cargarRemitoExterno = (file: File) => leerArchivo(file, async (archivo) => {
    ...
    try {
      ...
      if (v?.ok && v.data) { validacion = v.data; const d = v.data; let msg = `📄 ${d.observacion || "Documento leído"}.\n`; msg += d.tiene_firma_o_sello ? "• ✅ Tiene firma/sello." : "• ⚠️ No se detecta firma ni sello."; msg += "\n\n¿Cargar como remito del transporte y marcar el pedido DESPACHADO?"; if (!confirm(msg)) return; }
    } catch { /* sin IA → carga sin validar */ }
    onRemitoExterno(archivo, validacion);
  });
